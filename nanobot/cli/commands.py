...
def gateway(
    ...
    if config.copilot.enabled:
        ...
        try:
            ...
            embedder = Embedder(
                api_base=(config.providers.vllm.api_base or "http://localhost:1234/v1"),
    ...
    if config.copilot.enabled and config.copilot.slm_queue_enabled and extractor and memory_manager:
        ...
        _lm_url = (config.providers.vllm.api_base or "http://localhost:1234").rstrip("/v1").rstrip("/")
    ...
    if config.copilot.enabled:
        try:
            ...
            status_aggregator = StatusAggregator(
                db_path=str(db_path),
                lm_studio_url=config.providers.vllm.api_base or "http://localhost:1234",
                qdrant_url=config.copilot.qdrant_url,
                memory_manager=memory_manager,
...
def backfill_extractions(
    ...
    session_key = fpath.stem  # e.g. whatsapp_1234567890@s.whatsapp.net
    pending_user = None
    for line in lines:
        try:
