    ...
    def _load_store(self) -> CronStore:
        ...
        if self.store_path.exists():
            try:
                ...
                asyncio.ensure_future(get_alert_bus().alert("cron", "medium", f"Cron store load failed: {e}", "store_load_failed"))
        ...
        for job in self._store.jobs:
            if job.enabled:
                if job.schedule.kind == "at":
                    # One-shot jobs: if past-due, fire immediately instead of discarding.
                    # _compute_next_run returns None for past-due "at" jobs, which would
                    # silently kill them on restart.
                    if job.state.next_run_at_ms and job.state.next_run_at_ms <= now:
                        job.state.next_run_at_ms = now + 1000  # fire on next tick
                    # else: future time or no stored time — leave as-is or compute normally
